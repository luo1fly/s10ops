...
class Asset(models.Model):
    ...
    admin = models.ForeignKey(hosts_models.UserProfile, related_name='admin_user', verbose_name=u'资产管理员', null=True, blank=True)
...
class RAM(models.Model):
    """
    :param asset :内存和资产的关联关系是多对一的，因为一个资产（对应一台服务器或者一个交换机）可以有多根内存
    :param sn & unique_together : 内存的sn程序可能抓不到，相同的内存可以不唯一，但是同一插槽的内存是唯一的，所以做一个unique_together
    """
...
class NIC(models.Model):
    """
    :param sn & mac 对于网卡而言mac地址是唯一的，sn可能抓不到，允许为空
    """
    asset = models.ForeignKey('Asset')
